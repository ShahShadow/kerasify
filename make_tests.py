...
def output_testcase(model, test_x, test_y, name, eps):
    ...
    with open('test_%s.h' % name, 'w') as f:
        x_shape, x_data = c_array_init(test_x[0])
        y_shape, y_data = c_array_init(predict_y[0])
...
def output_testcase_ex(model, test_x_list, test_y_list, name, eps):
    ...
    with open('test_%s.h' % name, 'w') as f:
        x = ["Tensor(%s, %s)" % c_array_init(test_x[0]) for test_x in test_x_list]
        x = ',\n        '.join(x)
        y = ["{%s, %s}" % c_array_init(predict_y) for predict_y in predict_y_list]
        y = ',\n        '.join(y)

        input_layer_names = ', '.join(["\"%s\"" % layer_name for layer_name in model.input_names])
        output_layer_names = ', '.join(["\"%s\"" % layer_name for layer_name in model.output_names])
        f.write(TEST_CASE_EX % (name, name, input_layer_names, output_layer_names, x, y, name, eps))
...
output_testcase_ex(model, test_x_list, test_y_list, 'functional_dense_1x1_multi_in', '1e-6')
...
output_testcase_ex(model, test_x_list, test_y_list, 'functional_dense_1x1_multi_in_out', '1e-6')
...
output_testcase_ex(model, test_x_list, test_y_list, 'functional_dense_1x1_multi_out', '1e-6')
...
output_testcase_ex(model, [test_x], [test_y], 'functional_conv_2x2', '1e-6')

''' Functional Conv 2x2 Multi In Out'''
test_x_list = [np.random.rand(10, 1, 2, 2).astype('f'), np.random.rand(10, 1, 2, 2).astype('f')]
test_y_list = [np.random.rand(10, 1).astype('f'), np.random.rand(10, 1).astype('f')]
...
output_testcase_ex(model, test_x_list, test_y_list, 'functional_conv_2x2_multi_in_out', '1e-6')
